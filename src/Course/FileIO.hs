...
main =
  getArgs >>= \case
    path :. Nil -> run path
    _ -> putStrLn "Please provide a filepath"
...
sequence_ :: Monad k => List (k a) -> k ()
sequence_ = void . sequence

mapM :: Monad k => (a -> k b) -> List a -> k (List b)
mapM f = sequence . (<$>) f

mapM_ :: Monad k => (a -> k b) -> List a -> k ()
mapM_ f = foldRight (\x k -> f x >> k) (pure ())
